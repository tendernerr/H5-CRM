<template>
    <div>
        <van-nav-bar title="编辑员工" left-arrow @click-left="onClickLeft" />
        <div>
            <div style="background-color:#FFF;line-height: 30px;">
                ...
                <div style="background-color: #FFFF">
                    ...
                    <van-field v-model="post" is-link readonly label="请选择" placeholder="请选择" @click="show = true" />
            ...
            <div class="addDiv">
                <div class="add" @click="addClick">编辑</div>
            </div>
        </div>
    </div>
</template>
...
<script >
import { basicStaff, editstaff, classifyIndex } from '@/api/user'
...
export default {
    ...
    methods: {
        ...
        onFinish({ selectedOptions }) {
            this.show = false;
            this.post = selectedOptions.map((companyscale) => companyscale.name).join('/');
        },
        addClick() {
            editstaff({
                ...
                if (!phoneReg.test(this.phone)) {
                    ...
                    if (res.data.code == 200) {
                        this.$router.push("/workbench/allemplo")
                    }
                }
            })
        },
        ...
        basicStaffList() {
            // this.$route.query.id
            basicStaff({ id: this.$route.query.id }).then(res => {
                console.log(res);
                ...
                this.purchasetime = res.data.data.entry_time
            })
        }
    }
}
