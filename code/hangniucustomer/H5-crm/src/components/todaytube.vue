<template>
    <div>
        <van-nav-bar title="今日待办" left-arrow @click-left="onClickLeft" />
        <div style="background-color: #fff;margin-top: 10px;" v-for="item in today">
            <van-checkbox-group v-model="result" ref="checkboxGroup">
                <div class="Initial_op">
                    ...
                    <div class="img_op" @click="dial(item.contact_phone)">
...
<script>
import { todayNoFollow } from '@/api/user'
export default {
    props: {
        gettoday: {
            type: Array,
    ...
    watch: {
        gettoday(newValue, oldValue) {
            this.today = this.gettoday;  //赋值
        }
    },
    todayNoList() {
            todayNoFollow({
                ...
                this.todayList = res.data.data.list
            })
        },
    methods: {
        onSearch(val) {
