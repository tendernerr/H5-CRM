     ...
     <div class="service">
        ...
        <div class="selectArr">
            ...
            <div class="selectInput" v-for="(item,index) in param.endmoney" :key="index">
                ...
                <input placeholder="无内容将不会收到对应提醒" class="input" v-model="param.endmoney[index].text" type="text">
            </div>
        </div>
        <div class="selectArr">
            <div class="selectText">
...
export default {
    ...
    computed:{
        verification(){
            ...
            for (let i = 0; i < param.maintaininfo.length; i++) {
                if(param.maintaininfo[i].time !== '请选择日期' && param.maintaininfo[i].text){
                    isMaintaininfo = true
                }
            }
            let isWarrantytime = false
            for (let g = 0; g < param.warrantytime.length; g++) {
                if(param.warrantytime[g].time !== '请选择日期' && param.warrantytime[g].text){
                    isWarrantytime = true
                }
            }
    ...
    methods:{
        ...
        confirmTime(date){
            ...
            let week = weekArr[date.getDay()];  //当前星期几
            // 给一位数的数据前面加 “0”
        ...
        updateAfterSale(){
            ...
            http.get(api.updateAfterSale,{id}).then(res=>{
                let data = res.data
                data.maintaininfo = this.param.maintaininfo.map((ite,inde)=>{
                    if(data.maintaininfo[inde] !== undefined){
                        return data.maintaininfo[inde]
                    }
                    return ite
                })
                data.warrantytime = this.param.warrantytime.map((ite,inde)=>{
                    if(data.warrantytime[inde] !== undefined){
                        return data.warrantytime[inde]
                    }
                    return ite
                })
                data.othertip = this.param.othertip.map((ite,inde)=>{
                    if(data.othertip[inde] !== undefined){
                        return data.othertip[inde]
                    }
                    return ite
                })
                data.endmoney = this.param.endmoney.map((ite,inde)=>{
                    if(data.endmoney[inde] !== undefined){
                        return data.endmoney[inde]
                    }
                    return ite
                })
                data.consumables = this.param.consumables.map((ite,inde)=>{
                    if(data.consumables[inde] !== undefined){
                        return data.consumables[inde]
                    }
                    return ite
                })
                data.changetime = this.param.changetime.map((ite,inde)=>{
                    let changetime = {time:data.changetime?data.changetime:'请选择',text:''}
                    return changetime
                })
                data.id = id
        ...
        submit(){
            ...
            http.post(api.updateAfterSale,param).then(res=>{
...
<style lang="scss" scoped>
    .main{font-size: 16px; color: #000;
        ...
        .inputArr{ display: flex; align-items: center;padding: 20px 0 0 0;font-size: 14px;color: #7b7b7b;
                .inputDiv{width: 5.2em; text-align-last: justify; padding: 0 5px 0 0 ; color: #000;}
