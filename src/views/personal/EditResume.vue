<template>
  <div id="app">
    ...
    <div class="content_wrapper">
      ...
      <div class="box_3" v-if="moduleStore.intention!==undefined && moduleStore.intention.is_display == 1">
        ...
        <div class="box_content">
          ...
          >
            <div class="name">
             所在地区： [{{ item.district_text }}] {{ item.category_text }}
            </div>
            <div class="intent">
               <div>产品名称：{{basic.idcard}}</div>
               <div>截止日期：{{basic.birthday}}</div>
      ...
      <div class="box_13">
        <van-button round type="info" block @click="$router.push('/member/personal/preview')">预览项目</van-button>
      </div>
    </div>
    <div
      v-if="!showMoreBar"
    ...
    <van-dialog v-model="showCloseResume" title="项目设置" show-cancel-button>
      <div class="box_log_privacy">
        <div class="self_remind">
          项目设置为关闭后企业将无法搜索到您的项目哦
        </div>
  ...
  methods: {
    ...
    onConfirmCurrent (value) {
      ...
      http
        ...
        .then(res => {
          if (parseInt(res.code) === 200) {
            this.$notify({type: 'success', message: '成功修改采购状态'})
