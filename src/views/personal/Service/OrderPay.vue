<template>
  <div id="app">
        ...
        <div class="box_1">
          <div class="item">
            <div class="tx1">
              ...
              <div class="des">专享项目推荐特权</div>
            </div>
				<div class="changeSpan1"  @click="changeSpanBorder">
					{{selectText==''?'请选择您要置顶的项目':selectText}}
					<span :class="{'changeSpan2':changeSpan}"></span>
				</div>
          </div>
        </div>
      </div>
      ...
      <div class="box_wrapper">
        <div class="box_2">
          ...
          <div class="item" @click="useDeduct = !useDeduct" v-else>
            ...
            <van-checkbox class="self_check" v-model="useDeduct" icon-size="14px" @change="handlerDeduct"></van-checkbox>
          </div>
          <div class="item">
            应付金额
            <div class="right_txt">
        ...
        <div class="box_3">
          <van-button type="info" round block @click="paymentButton"
            >立即支付</van-button
          >
        </div>
      </div>
    ...
    <PaySubmit ref="paySubmit" :payment="submitData.payment" success-url="/member/order/list"></PaySubmit>
	 ...
	 <van-popup v-model="changeSpan" position="bottom">
	   <van-picker value-key="fullname" show-toolbar title="发布的项目" :columns="columns" :default-index="defaultIndex"  @confirm='confirm'/>
	 </van-popup>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    if (this.type == 'stick') {
      this.type_tip = '项目曝光量提升5倍，专享优先推荐特权，采购快人一步!'
    } else {
      this.type_tip = '专属项目标签点亮你的个性优势，是金子就要发光！!'
    }
    ...
    this.$store.dispatch('getClassify', 'resumeStrongTag')
	 ...
	 if(id){
		 this.submitData.rid = id
	 }
	 console.log(this.$route,"11111111111111111")
	 this.personalServiceMyProject(id)
  },
  methods: {
    fetchServiceList () {
      http
	 ...
	 changeSpanBorder(){
		 this.changeSpan = !this.changeSpan
	 },
	 personalServiceMyProject(id){
		 http.get(api.personalServiceMyProject,{}).then(res=>{
			 if(id){
				 for (let i = 0; i < res.data.length; i++) {
					 console.log(i,"iiiiiiiiiiiii")
				 	if(res.data[i].id == id){
						this.selectText = res.data[i].fullname
						this.defaultIndex = i
						break
				 	}
				 }
	 ...
	 paymentButton(){
		  if(this.submitData.rid == ''){
			  this.$notify({ type: 'warning', message: '请输入您要置顶的项目' });
			  return
		  }
	 ...
	 confirm(e){
		 console.log(e)
		 this.selectText = e.fullname
		 this.submitData.rid = e.id
		 this.changeSpan = false
	 },
    handlerSetServiceid (item) {
			console.log(item,"是见识见识见识见识")
...
<style lang="scss" scoped>
	.changeSpan1{width: 100%;height: 30px;margin-bottom: 10px; border-radius: 5px; font-size: 13px; line-height: 30px; text-indent: 2em;position: relative; border: 1px solid #d4d4d4; color: red;
		span{border-bottom: 1px solid #000; border-left:1px #000 solid; height: 10px; width: 10px; display: inline-block; position: absolute; right: 10px;top: 8px; transform: rotate(-46deg); transition: all 2s;}
		.changeSpan2{transform: rotate(135deg); top: 12px;}
	}
.box_custom {
  width: 290px;
  margin: 0 auto;
