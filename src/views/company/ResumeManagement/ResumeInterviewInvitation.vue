      ...
      <div class="box_1">
        ...
        >
          <div class="box_item">
            ...
            <div class="tx2">
				  <span style="color: #e79a00;">{{ item.birthday+'截止' }}</span> · <span style="color: #0055ff;">{{ item.education_text }}</span>
				  <br/><br/>
              {{ item.householdaddress_name }}
              <div class="right_txt" v-if="item.is_look == 1">已读</div>
              <div class="right_txt not_download" v-else>未读</div>
            </div>
            <div class="tx3" @click.stop="handlerDetail(item)">
              <div class="name">设备名称：{{ item.jobname }}</div>
              <div class="time_txt">
                报价时间：{{ item.interview_time | interviewTimeFilter }}
              </div>
            </div>
          </div>
          <div class="form_split_10"></div>
  ...
  data () {
    return {
      ...
      navList: [
        ...
        { text: '我的报价', href: '/member/company/interview', active: true },
      ...
      http
        ...
        .then(res => {
          if (init === true) {
            ...
            this.options_job = []
            this.options_job.push({ text: '全部案例', value: 0 })
