<template>
  <div id="app">
    ...
    <van-form @submit="onSubmit">
      ...
      required
		...
		<van-field
		  ...
		  label="擅长工艺"
		  placeholder="请选择"
		  @click="workmanship = true"
		  class="form_choose reset_after"
		/>
		
		
		
		<!-- <van-field v-model="item" required v-for="(item,index) in main_product"
		  name="mobile"
		  type="number"
		  label="联系电话"
		  placeholder="请填写"
		...
		<div class="main_product">
			<div class="main_product-box" style="position: relative;"><span style="color: red;position: absolute;left: 17px;">*</span>主营产品</div>
			<div class="main_product-inpu-box">
				<div class="main_product-inpu-input" v-for="(item,index) in main_product" :key="item.id">
					<input v-model="item.name" />
				</div>
				<span class="main_product-inpu-box-span" @click="main_product.length <6 ?main_product.push({name:'',id:new Date().getTime()}):$notify({ type: 'warning', message: '不能大于6个选项' })">添加主营产品+</span>
			</div>
		</div>
    ...
    <van-popup v-model="showPickerTrade" position="bottom">
	 ...
	 <van-popup v-model="jobPosition" position="bottom">
	   <van-picker
	     show-toolbar
		  value-key='name'
	     :columns="jobPositionList"
	     :defaultIndex="jobPositionIndex"
	     @confirm="jobPositionObjConfirm"
	     @cancel="jobPosition = false"
	   />
	 </van-popup>
...
export default {
  ...
  methods: {
    ...
    hanlderWeixin () {
	 ...
	 // 点击确认
	 jobPositionObjConfirm(v){
		 console.log(v)
		 this.postText = v.name
		 this.post = v.id
        ...
        .then(res => {
          ...
          if (basic.companyname === undefined || basic.companyname == '') {
            this.companynameDisabled = false
			 ...
			 this.post = res.data.info.post;//    在职岗位
			 this.postText = res.data.info.postText;//在职岗位名称
			 this.householdaddress = res.data.info.householdaddress;  // 擅长工艺
			 this.main_product = res.data.info.main_product; // 主营产品
          this.basic = {
            logo: basic.logo,
    ...
    for (let index = 0; index < this.main_product.length; index++) {
      if(this.main_product[index].name){
          arrs.push(this.main_product[index])
      }
    }
    console.log(arrs,"00000000000")
    if (arrs.length < 2) {
		  this.$notify('请添加至少两项主营产品')
		  return false
		}


		let main_product = []
		let arr = JSON.parse(JSON.stringify(this.main_product))
		for (var i = 0; i < arr.length; i++) {
			if(arr[i].name != ''){
				main_product.push(arr[i].name)
			}
		}
      http.post(api.company_profile, {
          basic: this.basic,
          info: this.info,
          contact: this.contact,
			 post:this.post,   								 //在职岗位
			 householdaddress:this.submitExperience,   //擅长工艺
			 main_product,  									//主营产品
        })
        .then(res => {
          this.$notify({ type: 'success', message: res.message })
			 this.$router.replace('/member/company/manage')
          // window.history.length > 1 ? this.$router.go(-1) : this.$router.push('/member/company/index')
        })
        .catch((res) => {
...
<style lang="scss" scoped>
	.main_product{font-size: 15px; min-height: 50px; border-bottom: 1px solid rgba(245, 245, 245, 1); display: flex; padding-right: 15px;
		.main_product-box{padding: 13px 0 0 26px; color: #000;width: 120px; }
		.main_product-inpu-box{display: flex; flex-direction: column;flex: 1;
			.main_product-inpu-input{ border-bottom: 1px solid #d6d6d6; padding: 13px 0;
				input{width: 100%; height: 100%; border: 0;}
			}
			.main_product-inpu-box-span{color: #08d2ff; padding: 10px 0;}
		}
	}
