<template>
  <div id="app" class="my_app">
    ...
    <van-skeleton title avatar :row="10" :loading="mainLoading">
      <van-swipe :loop="false" height="335" width="100%">
        <van-swipe-item v-for="(image, index) in base_info.caseimg" :key="index">
          <img @click="maxImgs()" v-if="image.isPic" width="100%" height="100%" v-lazy="image.img_src" />
          <video height="100%" width="100%" v-if="!image.isPic" controls>
            <source :src="image.img_src" type="video/mp4">
            <source :src="image.img_src" type="video/ogg">
          </video>
        </van-swipe-item>
      </van-swipe>
      <div class="box_1">
    ...
    justify-content: space-between;">
          ...
          <div class="chat_bar">
            {{ base_info.click }}人查看 更新：{{base_info.refreshtime}}
      ...
      <div class="box_3" v-if="base_info.address != ''">
        ...
        <div class="address">{{ base_info.address }}</div>
        <div class="bg">
      ...
      <div class="box_4" style="border-bottom:rgba(243,243,243,1) 5px solid;padding-bottom: 8px;">
        <div class="put">基本信息</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">设备类别：{{base_info.category_text}}</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">设备型号：{{base_info.equipment_model}}</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">设备交期：{{base_info.delivery_date}}</div>
      </div>
      <div class="box_4" style="border-bottom:rgba(243,243,243,1) 5px solid;padding-bottom: 8px;">
        <div class="put">设备特点</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;" v-for="(item,index) in base_info.equipment_characteristics" :key="index">
          {{index+1}}.{{item}}
        </div>
      </div>
      <div class="box_4" style="border-bottom:rgba(243,243,243,1) 5px solid;padding-bottom: 8px;">
        <div class="put">成功案例</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">客户公司名称:{{base_info.success_case&&base_info.success_case.name}}</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">客户地点:{{base_info.success_case&&base_info.success_case.address}}</div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">主营产品:<span style="margin-right:8px" v-for="(item,index) in base_info.success_case&&base_info.success_case.main_product" :key="index">{{item}}</span></div>
        <div style="padding-left: 10px;font-size: 14px;padding-bottom: 8px;">产品图片:</div>
        <img @click="maxImg(index)" style="padding-left: 10px;padding-bottom: 8px;" width="80" height="80" v-for="(item,index) in base_info.success_case&&base_info.success_case.img" :src="item.img_src" :key="index" />
      </div>
      ...
      <!-- <div class="box_5">
        <div class="put">案例动态</div>
        <div class="content">
          <div class="item">
            <div class="tx1">{{ watch_percent }}</div>
            <div class="tx2">企业项目查看率</div>
          </div>
          <div class="item">
      ...
      <div class="box_8">
        <div class="put">
          ...
          <span
            class="phone_tip"
            v-if="show_contact == 1 && phone_protect_open && phone_protect_type==1"
          >
            请使用
            <span class="phone" v-text="cur_user_mobile"></span> 的手机号拔号
          </span>
        </div>
        ...
        <div class="code_pro_wrap" v-if="show_contact == 1 && phone_protect_open">
          ...
          <div v-if="phone_protect_type==1" class="pro_tip">
            ...
            <span v-text="phone_protect_timeout"></span>秒,过期后需再次点击拔号
          </div>
        </div>
        <div
          class="contact_login"
        ...
        >
          <div class="tx1">案例联系方式</div>
          <div class="tx2">仅对采购方会员开放</div>
        </div>
        <div
          class="contact_resume"
        ...
        >
          <div class="tx1">您还没有项目</div>
          <div class="tx2">
            <span class="link">创建项目</span>
            后可查看企业联系方式
          </div>
        </div>
        <div
        ...
        >
          ...
          <div class="tx2" v-if="has_apply==1">您已投递案例，请等待企业联系</div>
          <div class="tx1" v-if="has_apply==0">企业未开启查看联系方式</div>
          <div class="tx2" v-if="has_apply==0">
            请直接
            <span class="link" @click="doApply">投递案例</span>
          </div>
        </div>
      </div>
      <div class="form_split_10"></div>
    ...
    <baidu-map class="bm-view" :ak="$store.state.config.map_ak" @ready="handlerMap"></baidu-map>
    ...
    >
      <JobCompetitive :info="competitive_data" @closeCompetitive="closeCompetitive"></JobCompetitive>
    </van-popup>
    <van-popup
      :lazy-render="false"
      v-model="showTipoff"
    ...
    >
      ...
      <div class="line18 font12">
        (电话
        <span class="color-orange" v-text="codePro.timeout"></span>秒后失效,请尽快拔打)
      </div>
      <div v-if="phone_protect_type==1" class="m-btm line18 font12 color-gray">
        仅支持使用
        <span v-text="codePro.a"></span>的手机卡拔号
      </div>
    </van-dialog>
    <div class="return_list" v-if="isRetrunBtn != null" @click="$router.push('/campus/job')">返回列表</div>
    <!-- <div class="click_copy" @click="handlerCopy">
      一键
      <br />复制
    </div> -->
    <!-- <div class="generate_posters" @click="handlePoster">
      生成
      <br />海报
    </div> -->
    <!-- 绑定微信开始 -->
...
供应商：${this.base_info.jobname}
要求：案例涉及工艺${this.base_info.experience_text}、项目类型要求${this.base_info.education_text}
工资：${this.base_info.wage_text}
查看联系方式：${this.shortUrl}
-供应商采购就上${this.$store.state.config.sitename}-`;
    ...
    async fetchData(next_method = null) {
      ...
      for (let i = 0; i < base_info.success_case.img.length; i++) {
        this.imgArr.push(base_info.success_case.img[i].img_src)
      }
      for (let s = 0; s < base_info.caseimg.length; s++) {
        if(base_info.caseimg[s].img_src.indexOf('.mp4') < 0){
          this.imgArrs.push(base_info.caseimg[s].img_src)
        }
      }
    ...
    async doTel() {
      if (this.show_contact === 1) {
        ...
        if (this.show_contact_note === "need_resume") {
          this.$notify("您还没有项目，创建项目后可拨打企业电话");
        } else if (this.show_contact_note === "need_apply") {
          this.$notify("您还没有投递案例，请投递案例后拨打企业电话");
        } else if (this.show_contact_note === "company_close") {
          if (this.has_apply == 1) {
            this.$notify("您已投递案例，请等待企业联系");
          } else {
            this.$notify("企业未公开联系方式，请直接投递案例");
          }
        }
      }
    },
