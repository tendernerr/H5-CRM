<template>
	<div id="app">
              ...
              time: item.recommend == 1
						...
						<div class="tx2">
							<span class="unit">￥</span>{{ item.expense1 }}.<span
								class="tofix">{{ item.expense2 }}</span>
						</div>
		...
		<div class="box_4" v-if="params.type == 'job_refresh'" @click="showDatetimePicker = true">
			<div class="content">
				开始刷新时间
				<div class="right_txt no_arrow">
		...
		<div class="box_4" v-if="params.type == 'job_refresh'" @click="showTimerangePicker = true">
			<div class="content">
				刷新间隔
				<div class="right_txt no_arrow">
					{{ refresh_timerange }}
		...
		<div class="box_5">
			<div class="content">
				...
				<van-button color="linear-gradient(to right, #f1cd91, #dab577)" text-color="#92672c" round size="small"
					@click="submit">{{ amount == 0 ? "立即兑换" : "立即支付" }}</van-button>
			</div>
		</div>
		<van-popup v-model="showPicker" round position="bottom">
			<van-picker show-toolbar :columns="joblist" @confirm="chooseJob" @cancel="showPicker = false" />
		</van-popup>
		<van-popup v-model="showDatetimePicker" round position="bottom">
			<van-datetime-picker v-model="refresh_starttime" type="datetime" title="请选择开始刷新时间"
				@confirm="handlerRefreshStarttime" :min-date="minDate" :formatter="formatter" />
		</van-popup>
		<van-popup v-model="showTimerangePicker" round position="bottom">
			<van-picker show-toolbar :columns="options_timerange" @confirm="handlerTimerange" />
		</van-popup>
...
<script>
	...
	export default {
		...
		created() {
			console.log(this.resumePackage,"resume_packageresume_packageresume_packageresume_package")
			...
			this.fetchData()
			console.log(this.resumePackage,"resume_packageresume_packageresume_packageresume_package")
			if(this.resumePackage){
				console.log(this.resumePackage,"1110121321541resume_packageresume_packageresume_packageresume_package")
				this.changeType(this.resumePackage)
			}
		},
		methods: {
			initDB() {
