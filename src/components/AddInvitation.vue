  ...
  <div id="app">
    ...
    <van-form @submit="onSubmit">
      <van-field
        ...
        v-model="form.type"
        label="报价形式"
        :rules="[{ required: true, message: '请选择报价形式' }]"
      >
        <template #input>
          <van-radio-group
            v-model="form.type"
            ...
            icon-size="14px"
          >
            <van-radio name="1">我的报价</van-radio>
            <van-radio name="2" :disabled="mySetmeal.enable_video_interview===0?true:false">视频报价</van-radio>
          </van-radio-group>
        </template>
      </van-field>
      <van-field
      ...
      <van-field
        ...
        placeholder="请填写"
        class="reset_after"
        :rules="[{ required: true, message: '请填写报价地点' }]"
      />
    ...
    onSubmit (values) {
      if (this.from === 'apply') {
        ...
        http
          ...
          .then(res => {
            this.$notify({ type: 'success', message: '邀请报价成功' })
            setTimeout(() => {
